...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['desfazer'])) {
    ...
    if ($_confronto !== null) {
        ...
        if ($desafiadoRatingOriginal !== null && $desafianteRatingOriginal !== null) {
            ...
            $_updateP1Query = "UPDATE t_atletas SET a_rating_atual = ?, a_vitorias = a_vitorias - ? WHERE a_index = ?";
            $_updateP1Stmt = $conn->prepare($_updateP1Query);
            $_updateP1Stmt->bind_param("idi", $desafiadoRatingOriginal, $_p1Pontos, $_confronto['a_desafiado']);

            if (!$_updateP1Stmt->execute()) {
                echo "Erro ao atualizar o rating do desafiado. Por favor, tente novamente. Erro: " . $_updateP1Stmt->error;
            }

            $_updateP2Query = "UPDATE t_atletas SET a_rating_atual = ?, a_vitorias = a_vitorias - ? WHERE a_index = ?";
            $_updateP2Stmt = $conn->prepare($_updateP2Query);
            $_updateP2Stmt->bind_param("idi", $desafianteRatingOriginal, $_p2Pontos, $_confronto['a_desafiante']);
    ...
    // Obter o rating atual dos atletas
    $desafiadoRatingAtual = obterDadosJogador($_confronto['a_desafiado'])['a_rating_atual'];
    $desafianteRatingAtual = obterDadosJogador($_confronto['a_desafiante'])['a_rating_atual'];
    
    $_p1NewRating = $desafiadoRatingAtual;
    $_p2NewRating = $desafianteRatingAtual;
    ...
    // o $resultado pode ser "desafiado", "desafiante" ou "empate"  escreva uma logica que atualize o rating dos jogadores sendo que o Calc precisa receber o rating1, rating2 e o resultado que pode ser 1, 0.5 ou 0 dependendo se o desafiado ou desafiante ganhou e deve calcular separadamente o rating de cada um
    switch ($resultado) {
        case 'desafiado':
            $_p1NewRating = Calc($desafiadoRatingAtual, $desafianteRatingAtual, 1);
            $_p2NewRating = Calc($desafianteRatingAtual, $desafiadoRatingAtual, 0);
            $_p1Pontos = 1;
            break;
        case 'desafiante':
            $_p1NewRating = Calc($desafiadoRatingAtual, $desafianteRatingAtual, 0);
            $_p2NewRating = Calc($desafianteRatingAtual, $desafiadoRatingAtual, 1);
            $_p2Pontos = 1;
            break;
        case 'empate':
            $_p1NewRating = Calc($desafiadoRatingAtual, $desafianteRatingAtual, 0.5);
            $_p2NewRating = Calc($desafianteRatingAtual, $desafiadoRatingAtual, 0.5);
            $_p1Pontos = 0.5;
            $_p2Pontos = 0.5;
            break;
        default:
    ...
    $_p1RatingDiff = $_p1NewRating - $desafiadoRatingAtual;
    $_p2RatingDiff = $_p2NewRating - $desafianteRatingAtual;
    ...
    $_updateP1Query = "UPDATE t_atletas SET a_rating_atual = ?, a_vitorias = a_vitorias + ? WHERE a_index = ?";
    $_updateP1Stmt = $conn->prepare($_updateP1Query);
    $_updateP1Stmt->bind_param("idi", $_p1NewRating, $_p1Pontos, $_confronto['a_desafiado']);
    ...
    $_updateP2Query = "UPDATE t_atletas SET a_rating_atual = ?, a_vitorias = a_vitorias + ? WHERE a_index = ?";
    $_updateP2Stmt = $conn->prepare($_updateP2Query);
    $_updateP2Stmt->bind_param("idi", $_p2NewRating, $_p2Pontos, $_confronto['a_desafiante']);
    ...
    $updateQuery = "UPDATE t_confrontos SET a_resultado = ?, a_desafiado_pontos = ?, a_desafiante_pontos = ?, a_data_update = NOW() WHERE a_index = ?";
    $stmt = $conn->prepare($updateQuery);
    ...
    <div class="container">
        ...
        <div class="datagrid">
            ...
            <table>
                ...
                <tbody>
                    <?php
                    $lastUpdatedQuery = "SELECT a_index FROM t_confrontos WHERE a_resultado IS NOT NULL ORDER BY a_data_update DESC LIMIT 1";
                    $lastUpdatedResult = $conn->query($lastUpdatedQuery);

                    $lastUpdatedRow = $lastUpdatedResult->fetch_assoc();


                    $confrontosQuery = "SELECT * FROM t_confrontos ORDER BY a_index DESC";
                    ...
                    while ($row = $confrontosResult->fetch_assoc()) {
                        ...
                        if ($row['a_index'] === $lastUpdatedRow['a_index']) {
                            ...
                            echo "<td><a class='button-desfazer' href='atualizacao.php?desfazer={$row['a_index']}'><img src='template/img/desfazer.png' title='Desfazer' style='height: 1em; width: auto;'></a></td>";
                        } else {
